...
describe("Checks whether category exists or whether it is invlaid", () => {
  test("gets valid category", () => {
    return checkCategoryExists("dexterity").then((res) => {
      expect(res).toBe(true);
    });
  });
  test("gets invalid review id", () => {
    return checkCategoryExists("bananas").then((res) => {
      expect(res).toBe(false);
    });
  });
});
